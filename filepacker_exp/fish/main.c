...
int main() {
	#define FILENAME "img.png"
	#include "payload.h"

    #define orig_size 95
    unsigned char orig[orig_size] = {
	...
	FILE* payload_w = fopen("payload.png", "wb+");
    fwrite(payload, 1, payload_size, payload_w);
    fclose(payload_w);
    FILE* orig_w = fopen(FILENAME, "wb+");
    fwrite(orig, 1, orig_size, orig_w);
    fclose(orig_w);
}
